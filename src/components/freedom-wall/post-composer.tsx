...
*/
  ...
  const onFile = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_FILE) {
      toast.error("Please choose a file under 2 MB for this prototype.");
      return;
    }
    const reader = new FileReader();
    reader.onload = () =>
      setMedia({ type: "image", dataUrl: String(reader.result), name: file.name });
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (result.status === "pending")
      toast("Your note is safe with us", {
        description: "It needs a quick review before appearing on the wall.",
      });
    else
      toast.success("Your thought is on the wall!", {
        description: "Thanks for making this space more human.",
      });
  };
  return (
    <Dialog open={composerOpen} onOpenChange={setComposerOpen}>
      <DialogContent className="max-h-[92vh] overflow-y-auto border-border bg-background sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle className="font-display text-2xl">Share your thought</DialogTitle>
          <DialogDescription>Leave a kind, honest note for the CMU community.</DialogDescription>
        </DialogHeader>
        <form onSubmit={submit} className="space-y-5">
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <Label htmlFor="thought">Your message</Label>
              <span
                className={cn(
          ...
          <fieldset className="space-y-2">
            <legend className="text-sm font-medium">Pick a note color</legend>
            <div className="flex flex-wrap gap-3">
              {noteColors.map((item) => (
                <button
                  ...
                  className={cn(
                    "flex size-10 items-center justify-center rounded-full border-2 border-transparent shadow-sm transition-transform hover:scale-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring",
                    item.className,
                    color === item.id && "border-primary",
                  )}
                >
                  {color === item.id && <Check className="size-4 text-note-ink" />}
                </button>
              ))}
            </div>
          </fieldset>
          ...
          <div className="space-y-2">
            <Label>
              Photo or video{" "}
              <span className="font-normal text-muted-foreground">(optional, max 2 MB)</span>
            </Label>
            <input
              ref={fileRef}
              type="file"
            ...
            {media ? (
              <div className="flex items-center gap-3 rounded-md border bg-surface p-3">
                <div className="size-14 overflow-hidden rounded bg-muted">
                  {media.type === "image" ? (
                    ...
                    <video src={media.dataUrl} className="h-full w-full object-cover" />
                  )}
                </div>
                <span className="min-w-0 flex-1 truncate text-sm">{media.name}</span>
                <Button
                  type="button"
                  variant="ghost"
                  size="icon"
              ...
              >
                <ImagePlus /> Add from your device
              </Button>
            )}
          </div>
          <div className="flex gap-3 rounded-md bg-accent/60 p-3 text-sm text-accent-foreground">
            <Sparkles className="mt-0.5 size-4 shrink-0" />
            <p>
              <strong>Keep it kind.</strong> No names, bullying, threats, or private information.
